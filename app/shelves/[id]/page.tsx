  ...
  useEffect(() => {
    const fetchShelfDetails = async () => {
      try {
        ...
        const shelfData = responseData.data || responseData;

        // Transform the shelf data to match our frontend types
        const mappedShelf: Shelf = {
          _id: shelfData._id || '',
          name: shelfData.name || 'Unnamed Shelf',
          description: shelfData.description || '',
          type: ['product', 'service', 'investment'].includes(shelfData.type)
            ? shelfData.type as Shelf['type']
            : 'product',
          openForMembers: shelfData.openForMembers !== false,
          members: shelfData.members?.map((member: any) => ({
            userId: typeof member.userId === 'object' 
              ? {
          ...
          productDetails: shelfData.productDetails?.map((product: any) => ({
            name: product.name || '',
          ...
          serviceDetails: shelfData.serviceDetails?.map((service: any) => ({
          ...
          investmentDetails: shelfData.investmentDetails?.map((investment: any) => ({
            amount: investment.amount || 0,
            roi: investment.roi || 0,
            duration: investment.duration || '',
            riskLevel: ['low', 'medium', 'high'].includes(investment.riskLevel)
              ? investment.riskLevel as 'low' | 'medium' | 'high'
              : 'medium'
          })) || [],
          createdAt: shelfData.createdAt || new Date().toISOString(),
          updatedAt: shelfData.updatedAt || new Date().toISOString()
        };
        ...
        setError(error.message || "Failed to load shelf details");
        setLoading(false);
      }
    };
